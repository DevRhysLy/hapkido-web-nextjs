...
export default function Slug({
  ...
  return (
    ...
    >
      <Head>
        <title>
          {allAboutPages.fields.title} | Hapkido College of Australia
        </title>
      </Head>
      <Parallax
        image={`https:${allAboutPages.fields.bannerImage.fields.file.url}`}
        alt={allAboutPages.fields.title}
        responsive={true}
      >
        ...
        <div className={classes.container}>
          <div className={classes.jumboHeadingContainer}>
            <h1 className={classes.jumboHeading}>
              {allAboutPages.fields.title}
            </h1>
          </div>
          <ReactMarkdown className={style.reactMarkDown}>
            {allAboutPages.fields.pageContent}
          </ReactMarkdown>
        </div>
      </div>
    </Layout>
...
export async function getStaticPaths() {
  ...
  const paths = allAboutPages.map(({ fields: { slug } }) => ({
    params: { slug },
  }));
